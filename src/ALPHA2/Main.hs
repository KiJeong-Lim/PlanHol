...
runAnalyzer src0
    = case runHolLexer src0 of
        ...
        Right src1 -> case runHolParser src1 of
            Left (s, Nothing) -> Left ("*** parsing error at EOF, sym = " ++ shows s ".")
            Left (s, Just token) -> case getSLoc token of
                SLoc (row1, col1) (row2, col2) -> Left ("*** parsing error at { row = " ++ showsPrec 0 row1 (", col = " ++ showsPrec 0 col1 " }, sym = " ++ shows s "."))
